...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let typeF = '';
    let statusF = '';
    if (type) { typeF = `AND c.customer_type = $${idx}`; params.push(type); idx++; }
    if (status === 'active') statusF = `AND c.is_active = true`;
    else if (status === 'inactive') statusF = `AND c.is_active = false`;
    const whereBase = `
      FROM customers c
    ...
    const [res, countRes, summaryRes] = await Promise.all([
      db.query(`SELECT c.* ${whereBase} ORDER BY ${orderCol} ${sortDir} LIMIT $${idx} OFFSET $${idx + 1}`, [...params, limit, offset]),
